...
//        super.onAuthenticationSuccess(request, response, authentication);
        ...
        String refreshToken = tokenProvider.generateToken(user, REFRESH_TOKEN_DURATION); // tokenProvider로 리프레시 토큰 생성
        saveRefreshToken(user.getId(), refreshToken); // DB에 유저 아이디와 토큰 저장

        // 2. 액세스 토큰 생성 -> 패스에 액세스 토큰 추가
        String accessToken = tokenProvider.generateToken(user, ACCESS_TOKEN_DURATION); // 액세스 토큰 생성(tokenProvider)
        ...
        // 4. 리다이렉트
        getRedirectStrategy().sendRedirect(request, response, targetUrl); // 2의 URL(tragetUrl)로 리다이렉트
    }
